...
import type { IWithingsApi, WithingsClientAuthentication, WithingsClientOptions } from '~/models/withings-client.model';
...
export class WithingsClient extends BaseWithingsClient {
  ...
  authorizationUrl({
    ...
    this.auth.state = request.state;

    if (demo) request.mode = 'demo';

    return this.authentication.authorize(request);
  }
  ...
  private async _requestToken(request: WithingsAuthorizationCodeRequest | WithingsRefreshTokenRequest): Promise<WithingsClientAuthentication> {
    ...
    const token = new WithingsAuthToken(content.body);
